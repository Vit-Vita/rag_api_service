...
#CSV_PATH   = sys.argv[1] if len(sys.argv) > 1 else "jobs_clean.csv"
CSV_PATH  = "jobs_data/jobs_clean.csv"
BATCH_SIZE = 100
...
df = df[df["name"].str.lower() == "hager ag"]
...
batch = []
for _, row in tqdm(df.iterrows(), total=len(df), desc="⬆️  Upserting"):
    batch.append((
